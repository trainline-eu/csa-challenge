...
var isSameCityLoaded = false;
var timeTable = [];
var sameCityConnections = [];
const MAX_STATIONS = 20;
const TRANSFER_TIME_THRESHOLD = 900; //15 minutes

  
//first we get the connextions from stdin, then an empty line, then same city connection, then an empty line then one request at a time.
function processLine (line) { 
  ...
  }else if(!isSameCityLoaded){
    processSameCityConnection(line)
  }else{
    processRequest(line)
  }
}
...
function processSameCityConnection(line){
  //for more ease a use, we will push it twice, forward and backwards.
  var tokens = line.split(" ");
  sameCityConnections.push({ 
    departureStation : parseInt(tokens[0]),
    arrivalStation : parseInt(tokens[1]),
    travelTime : parseInt(tokens[2])
  });
  sameCityConnections.push({ 
    departureStation : parseInt(tokens[1]),
    arrivalStation : parseInt(tokens[0]),
    travelTime : parseInt(tokens[2])
  });
}
...
function computeRouteLeastConnections(lastStep, earliestArrivalMinConnections, route){
  ...
  if(selectedPossibility.connectionCount === -1){//found our starting point
    return route;
  }else{
    
    if(selectedPossibility.refersToTimetableIndex === null){
      route.push(selectedPossibility.fakeConnection);
    }else{
