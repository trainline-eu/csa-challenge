...
EOF
#assumes constant travel time from one station to the other
#time can be different between pairs of station : gare du nord <-> gare de l'est is much faster than gare de l'est <-> montparnasse.
  ...
  def test_start_from_same_city
    puts "test_start_from_same_city"
    @io.puts "16 18 60000"
    response = read_answer @io
    assert_equal 4, response.size
    assert_equal "EARLIEST_ARRIVAL", response[0][:solution_type]
    assert_equal 16, response[0][:departure_station]
    assert_equal 17, response[0][:arrival_station]
    assert_equal 60000, response[0][:departure_timestamp]
    assert_equal 62000, response[0][:arrival_timestamp]    
    assert_equal "EARLIEST_ARRIVAL", response[1][:solution_type]
    assert_equal 17, response[1][:departure_station]
    assert_equal 18, response[1][:arrival_station]
    assert_equal 65000, response[1][:departure_timestamp]
    assert_equal 67000, response[1][:arrival_timestamp]
    assert_equal "LEAST_CONNECTIONS", response[2][:solution_type]
    assert_equal 16, response[2][:departure_station]
    assert_equal 17, response[2][:arrival_station]
    assert_equal 60000, response[2][:departure_timestamp]
    assert_equal 62000, response[2][:arrival_timestamp]    
    assert_equal "LEAST_CONNECTIONS", response[3][:solution_type]
    assert_equal 17, response[3][:departure_station]
    assert_equal 18, response[3][:arrival_station]
    assert_equal 65000, response[3][:departure_timestamp]
    assert_equal 67000, response[3][:arrival_timestamp]
  end

  def test_ends_in_same_city
    puts "test_ends_in_same_city"
    @io.puts "15 17 60000"
    response = read_answer @io
    assert_equal 4, response.size
    assert_equal "EARLIEST_ARRIVAL", response[0][:solution_type]
    assert_equal 15, response[0][:departure_station]
    assert_equal 16, response[0][:arrival_station]
    assert_equal 61000, response[0][:departure_timestamp]
    assert_equal 62000, response[0][:arrival_timestamp]    
    assert_equal "EARLIEST_ARRIVAL", response[1][:solution_type]
    assert_equal 16, response[1][:departure_station]
    assert_equal 17, response[1][:arrival_station]
    assert_equal 62000, response[1][:departure_timestamp]
    assert_equal 64000, response[1][:arrival_timestamp]
    assert_equal "LEAST_CONNECTIONS", response[2][:solution_type]
    assert_equal 15, response[2][:departure_station]
    assert_equal 16, response[2][:arrival_station]
    assert_equal 61000, response[2][:departure_timestamp]
    assert_equal 62000, response[2][:arrival_timestamp]    
    assert_equal "LEAST_CONNECTIONS", response[3][:solution_type]
    assert_equal 16, response[3][:departure_station]
    assert_equal 17, response[3][:arrival_station]
    assert_equal 62000, response[3][:departure_timestamp]
    assert_equal 64000, response[3][:arrival_timestamp]
  end

  def test_goes_through_same_city
    puts "test_goes_through_same_city"
    @io.puts "15 18 60000"
    response = read_answer @io
    ...
    assert_equal 64000, response[1][:arrival_timestamp]
    assert_equal "EARLIEST_ARRIVAL", response[2][:solution_type]
    assert_equal 17, response[2][:departure_station]
    assert_equal 18, response[2][:arrival_station]
    assert_equal 65000, response[2][:departure_timestamp]
    assert_equal 67000, response[2][:arrival_timestamp]    
    assert_equal "LEAST_CONNECTIONS", response[3][:solution_type]
    assert_equal 15, response[3][:departure_station]
    assert_equal 16, response[3][:arrival_station]
    assert_equal 61000, response[3][:departure_timestamp]
    assert_equal 62000, response[3][:arrival_timestamp]    
    assert_equal "LEAST_CONNECTIONS", response[4][:solution_type]
    assert_equal 16, response[4][:departure_station]
    assert_equal 17, response[4][:arrival_station]
    assert_equal 62000, response[4][:departure_timestamp]
    assert_equal 64000, response[4][:arrival_timestamp]    
    assert_equal "LEAST_CONNECTIONS", response[5][:solution_type]
    assert_equal 17, response[5][:departure_station]
    assert_equal 18, response[5][:arrival_station]
    assert_equal 65000, response[5][:departure_timestamp]
    assert_equal 67000, response[5][:arrival_timestamp]
  end
